...
summary: 本節提供將資料從以 7 模式運行的NetApp Data ONTAP遷移到ONTAP的詳細步驟。 
---
= 從 7-模式到ONTAP 的資料遷移
:hardbreaks:
:allow-uri-read: 
...
[role="lead"]
本節提供將資料從以 7 模式運行的NetApp Data ONTAP遷移到ONTAP的詳細步驟。



== 將 7 模式 NFSv3 儲存過渡到ONTAP以用於 NFS 數據

本節提供了下表中將來源 7-模式 NFSv3 導出過渡到ONTAP系統的逐步過程。

NetApp假定來源 7-模式 NFSv3 磁碟區已匯出並安裝在用戶端系統上，且 XCP 已安裝在 Linux 系統上。
...
....
+
如果沒有資料聚合，請使用 `storage aggr create`命令。
...
....
+
您也可以使用下列指令指定磁碟區安裝選項（連線路徑） `volume create`命令。
...
2 entries were displayed.
....
. 連接到 Linux NFS 伺服器。為 NFS 匯出磁碟區建立掛載點。
+
....
[root@localhost /]# cd /mnt
[root@localhost mnt]# mkdir dest
....
. 在此掛載點掛載目標 NFSv3 匯出磁碟區。
+

NOTE: NFSv3 磁碟區應該被匯出但不一定由 NFS 伺服器掛載。如果可以安裝，XCP Linux 主機用戶端就會安裝這些磁碟區。
...
NOTE: 讀寫測試完成後，從目標NFS掛載點刪除該檔案。

. 連接到安裝了 XCP 的 Linux 用戶端系統。瀏覽到 XCP 安裝路徑。
+
....
[root@localhost ~]# cd /linux/
[root@localhost linux]#
....
. 透過執行以下命令查詢來源 7-模式 NFSv3 匯出 `xcp show`XCP Linux 用戶端主機系統上的指令。
+
....
...
....
. 掃描來源 NFSv3 匯出路徑並列印其檔案結構的統計資料。
+
NetApp建議在 xcp 期間將來源 NFSv3 匯出置於唯讀模式 `scan`， `copy` ， 和 `sync`營運.
...
....
. 將來源 7-模式 NFSv3 匯出複製到目標ONTAP系統上的 NFSv3 匯出。
+
....
...
....
. 複製完成後，驗證來源和目標 NFSv3 匯出是否具有相同的資料。運行 `xcp verify`命令。
+
....
...
....
+
如果 `xcp verify`發現來源資料和目標資料之間存在差異，則錯誤 `no such file or directory`在摘要中進行了報告。要解決該問題，請運行 `xcp sync`命令將來源變更複製到目標。

. 在切換之前和切換期間，運行 `verify`再次。如果來源有新的或更新的數據，則執行增量更新。運行 `xcp sync`命令。
+
....
...
....
. 若要恢復先前中斷的複製操作，請執行 `xcp resume`命令。
+
....
...
....
+
後 `resume`完成複製文件，運行 `verify`再次確保來源儲存和目標儲存具有相同的資料。

. NFSv3 用戶端主機需要卸載從 7-模式儲存配置的來源 NFSv3 匯出，並從ONTAP掛載目標 NFSv3 匯出。切換需要中斷。




== 將 7-模式磁碟區 Snapshot 副本過渡到ONTAP

本節介紹將來源 7-模式磁碟區NetApp Snapshot 副本過渡到ONTAP的過程。


NOTE: NetApp假定來源 7-模式磁碟區已匯出並安裝在用戶端系統上，且 XCP 已安裝在 Linux 系統上。 Snapshot 副本是磁碟區的某個時間點映像，用於記錄自上次 Snapshot 副本以來的增量變更。使用 `-snap`以 7 模式系統為來源的選項。

*警告：*保留基本 Snapshot 副本。基線複製完成後，請勿刪除基線 Snapshot 副本。進一步的同步操作需要基本 Snapshot 副本。
   ...
   Vserver Operational State Stopped Reason: -
            ...
            Is Vserver with Infinite Volume: false
                           QoS Policy Group: -
                               ...
                               IPspace Name: Default
....
. 從目標 SVM 中刪除 FCP、iSCSI、NDMP 和 CIFS 協定。
+
....
CLUSTER::> vserver remove-protocols -vserver dest -protocols fcp,iscsi,ndmp,cifs
...
. 掃描來源 NFSv3 匯出路徑並列印其檔案結構的統計資料。
+
NetApp建議在以下情況下將來源 NFSv3 匯出置於唯讀模式： `xcp scan` ， `copy` ， 和 `sync`營運.在 `sync`操作時，您必須通過 `-snap`具有相應值的選項。
...
(show scan and sync)
....
. 將來源 7-模式 NFSv3 快照（基礎）複製到目標ONTAP系統上的 NFSv3 匯出。
+
....
...
....
+
如果 `verify`發現來源資料和目標資料之間存在差異，則錯誤 `no such file or directory `is reported in the summary. To fix that issue, run the `xcp sync`命令將來源變更複製到目標。

. 在切換之前和切換期間，運行 `verify`再次。如果來源有新的或更新的數據，則執行增量更新。如果有增量更改，請為這些變更建立新的 Snapshot 副本，並將該快照路徑與 `-snap`同步操作的選項。
+
運行 `xcp sync`命令 `-snap`選項和快照路徑。
...
....
. NFSv3 用戶端主機必須卸載從 7-模式儲存配置的來源 NFSv3 匯出，並從ONTAP掛載目標 NFSv3 匯出。此切換需要停機。




== 將 ACLv4 從NetApp 7-模式遷移到NetApp儲存系統

本節介紹將來源 NFSv4 導出過渡到ONTAP系統的逐步過程。


NOTE: NetApp假定來源 NFSv4 磁碟區已匯出並安裝在用戶端系統上，且 XCP 已安裝在 Linux 系統上。來源應該是支援 ACL 的NetApp 7-模式系統。僅支援從NetApp到NetApp 的ACL 遷移。若要複製名稱中帶有特殊字元的文件，請確保原始檔案和目標檔案支援 UTF-8 編碼語言。



=== 將來源 NFSv4 導出移轉到ONTAP 的前提條件

將來源 NFSv4 匯出移轉到ONTAP之前，必須滿足以下先決條件：

* 目標系統必須配置 NFSv4。
* NFSv4 來源和目標必須安裝在 XCP 主機上。選擇 NFS v4.0 來匹配來源和目標存儲，並驗證來源和目標系統上是否啟用了 ACL。
* XCP 要求將來源/目標路徑掛載到 XCP 主機上以進行 ACL 處理。在下面的範例中， `vol1(10.63.5.56:/vol1)`安裝在 `/mnt/vol1`小路：
...
* 為了讓 XCP 在子目錄上運作 `(/vol1/DIR1/DIR11`）、掛載完整路徑(`10.63.5.56:/vol1/DIR1/DIR11`) 在 XCP 主機上。
+
如果沒有掛載完整路徑，XCP會回報以下錯誤：
...
....
* 使用子目錄語法(`mount: subdirectory/qtree/.snapshot`)，如下例所示：
...
....
完成以下步驟將 ACLv4 從NetApp 7-模式移轉到NetApp儲存系統。
...
[root@localhost mnt]# mkdir dest
....
. 在此掛載點掛載目標 NFSv4 匯出磁碟區。
+

NOTE: NFSv4 磁碟區應該被匯出但不一定由 NFS 伺服器掛載。如果可以安裝，XCP Linux 主機用戶端就會安裝這些磁碟區。
...
[root@localhost linux]#
....
. 透過執行以下命令查詢來源 NFSv4 匯出 `xcp show`XCP Linux 用戶端主機系統上的指令。
+
....
...
....
. 掃描來源 NFSv4 匯出路徑並列印其檔案結構的統計資料。
+
NetApp建議在以下情況下將來源 NFSv4 匯出置於唯讀模式： `xcp scan` ， `copy` ， 和 `sync`營運.
...
....
. 將來源 NFSv4 匯出複製到目標ONTAP系統上的 NFSv4 匯出。
+
....
...
....
. 後 `copy`完成後，驗證來源和目標 NFSv4 導出是否具有相同的資料。運行 `xcp verify`命令。
+
....
...
....
+
如果 `verify`發現來源資料和目標資料之間存在差異，則錯誤 `no such file or directory`在摘要中進行了報告。要解決該問題，請運行 `xcp sync`命令將來源變更複製到目標。
...
NOTE: 對於此操作，需要先前的複製索引名稱或編號。

. 恢復先前中斷的 `copy`操作，運行 `xcp resume`命令。
+
....
...
== 將 7-模式 SMB 儲存過渡到ONTAP以用於 CIFS 數據

本節介紹將來源 7-模式 SMB 共享過渡到ONTAP系統的逐步過程。


NOTE: NetApp假定 7-模式和ONTAP系統是 SMB 許可的。已建立目標 SVM、匯出來源和目標 SMB 共用，並已安裝和授權 XCP。
...
....
. 將檔案（有或沒有 ACL）從來源複製到目標 SMB 共用。以下範例顯示了具有 ACL 的副本。
+
....
...
NOTE: 如果沒有資料聚合，則使用儲存空間建立新的資料聚合 `aggr create`命令。
